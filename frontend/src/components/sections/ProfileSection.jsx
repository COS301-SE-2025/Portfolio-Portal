//frontend/src/components/sections/ProfileSection.jsx:
...
const ProfileEditForm = ({ profile, onUpdate, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    name: profile.name || "",
    bio: profile.bio || "",
    about: Array.isArray(profile.about_paragraphs)
      ? profile.about_paragraphs.join("\n\n")
      : "",
  ...
  const handleSubmit = async (e) => {
    ...
    const updatedData = {
      ...formData,
      about_paragraphs: formData.about
        .split("\n\n")
        .map((p) => p.trim())
        .filter(Boolean),
  ...
  const inputFields = [
    ...
    {
      id: "about",
      label: "About (each paragraph separated by a blank line)",
      type: "textarea",
      rows: 10,
    },
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end space-x-4 pt-6 border-t border-gray-700">
        ...
        <button
          ...
          disabled={isLoading}
          className="px-6 py-3 rounded-xl font-medium transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center bg-gradient-to-r from-purple-600 to-blue-600 text-white hover:from-purple-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <>
  ...
  const portfolioPlaceholders = [
    {
      ...
      image:
        "https://starwalk.space/gallery/images/what-is-space/1920x1080.jpg?w=300&h=200&fit=crop",
      link: "http://localhost:5173/space",
    },
    {
      title: "Forest themed Portfolio",
      image:
        "https://wallpaperonline.co.za/wp-content/uploads/2022/01/Screen-Shot-2020-11-04-at-00.17.25-e1632808578117.jpg?w=300&h=200&fit=crop",
      link: "http://localhost:5173/forest",
    },
    {
      title: "Office themed Portfolio",
      image:
        "https://digital-walls.com/cdn/shop/products/Globe.png?v=1666086840&width=533?w=300&h=200&fit=crop",
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div
          className={`absolute top-20 right-10 w-32 h-32 rounded-full blur-xl animate-float-slow ${
            isDark ? "bg-blue-500/15" : "bg-purple-300/30"
          }`}
        ></div>
        <div
          className={`absolute top-60 left-20 w-24 h-24 rounded-full blur-lg animate-float-medium ${
            isDark ? "bg-indigo-500/20" : "bg-blue-300/35"
          }`}
        ></div>
        <div
          className={`absolute bottom-40 right-40 w-20 h-20 rounded-full blur-lg animate-float-fast ${
