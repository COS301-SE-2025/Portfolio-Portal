...
const HowItWorksSection = forwardRef(
  ({ id, show, handleScrollToSection }, ref) => {
    const { isDark } = useTheme();

    return (
      <SectionWrapper id={id} show={show} ref={ref} isDark={isDark}>
        <div
          className={`relative z-10 max-w-4xl w-full text-center space-y-12 animate-fadeIn ${
            isDark ? "text-white" : "text-slate-900"
          }`}
        >
          <h2 className="text-5xl lg:text-6xl font-bold mb-8 relative">
            <span className="relative inline-block">How it</span>
            <br />
            <span
          ...
          <p
            className={`text-xl max-w-3xl mx-auto leading-relaxed mb-16 relative ${
              isDark ? "text-slate-300" : "text-slate-600"
            }`}
          >
            Portfolio Portal uses smart OCR and beautiful templates to turn your
            resume into a{" "}
            <span
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16">
            {steps.map((step) => (
              <div
                key={step.number}
                className="flex flex-col items-center space-y-6 relative group"
              >
                {/* step number circles (1, 2, 3) */}
                <div
                  className="relative w-20 h-20 rounded-full flex items-center justify-center text-white text-3xl font-bold transition-all duration-300 group-hover:scale-110 
    bg-gradient-to-br from-purple-500 to-blue-500 shadow-2xl hover:shadow-blue-500/30 
    transform hover:-translate-y-1"
          ...
          >
            ...
            <svg
              className={`relative z-10 w-12 h-12 transition-colors duration-300 ${
                isDark
