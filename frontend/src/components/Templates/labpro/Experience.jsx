    ...
    <section id="experience" className="relative w-full py-20 mx-auto bg-gray-900/20">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-red-400 text-4xl font-bold mb-4">Research Career</h2>
          <div className="w-20 h-1 bg-gradient-to-r from-red-400 to-green-400 mx-auto" />
        </div>

        {/* Experience Section */}
        <div className="space-y-8">
          {cvData?.experience?.map((exp, index) => {
            const borderColors = ['border-green-400/20', 'border-red-400/20', 'border-purple-400/20'];
            const textColors = ['text-green-400', 'text-red-400', 'text-purple-400'];
            return (
              <div 
                key={index}
        ...
        {cvData?.education?.length > 0 && (
          <div className="mt-20">
            <div className="text-center mb-16">
              ...
              <div className="w-20 h-1 bg-gradient-to-r from-purple-400 to-green-400 mx-auto" />
            </div>

            <div className="space-y-8">
              {cvData.education.map((edu, index) => {
                const borderColors = ['border-purple-400/20', 'border-green-400/20', 'border-red-400/20'];
                const textColors = ['text-purple-400', 'text-green-400', 'text-red-400'];
                return (
                  <div 
                    key={index}
                    className={`bg-gray-900/70 p-8 rounded-2xl backdrop-blur-sm ${borderColors[index % borderColors.length]} hover:${borderColors[index % borderColors.length].replace('/20', '/50')} transition-all duration-300`}
